        ...
        <div class="flex items-center justify-between mb-4">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">
                    Profil de {{ $craftsman->first_name }} {{ $craftsman->last_name }}
                </h1>
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('dashboard') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white rounded-lg shadow-md p-6 sticky top-8">
                ...
                <div class="text-center mb-6">
                    <div class="w-32 h-32 mx-auto bg-gradient-to-br from-blue-400 to-purple-600 rounded-full flex items-center justify-center mb-4">
                        <img src="{{ asset('images/default-craftsman-avatar.svg') }}" alt="Photo de profil de {{ $craftsman->first_name }} {{ $craftsman->last_name }}" class="w-28 h-28 object-contain rounded-full">
                    </div>
                    <div class="flex items-center justify-center gap-3 mb-2">
                        <h2 class="text-2xl font-bold text-gray-900">{{ $craftsman->first_name }} {{ $craftsman->last_name }}</h2>
                ...
                <!-- Spécialités -->
                @if($craftsman->specialty)
                    <div class="mb-6">
                        <h3 class="text-lg font-semibold text-gray-900 mb-3">Spécialité</h3>
                ...
                <!-- Matériaux -->
                @if($craftsman->material)
                    <div class="mb-6">
                        <h3 class="text-lg font-semibold text-gray-900 mb-3">Matériaux</h3>
                        <div class="flex flex-wrap gap-2">
                            @foreach(explode(',', $craftsman->material) as $materiau)
                ...
                <div class="border-t pt-6 mt-6">
                    ...
                    <div class="space-y-2">
                        @if($craftsman->email)
                            <div class="flex items-center text-sm text-gray-600">
                                ...
                                <a href="mailto:{{ $craftsman->email }}" class="hover:text-blue-600">{{ $craftsman->email }}</a>
                            </div>
                        @endif
                        @if($craftsman->phone)
                            <div class="flex items-center text-sm text-gray-600">
                                ...
                                <a href="tel:{{ $craftsman->phone }}" class="hover:text-blue-600">{{ $craftsman->phone }}</a>
                            </div>
                        @endif
                    ...
                    <div class="mt-4">
                        <a href="{{ route('chat.craftsman.start', $craftsman->id) }}" class="w-full bg-green-600 text-white px-4 py-3 rounded-md text-sm font-medium hover:bg-green-700 transition-colors flex items-center justify-center">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"></path>
                            </svg>
                            Contacter l'artisan
                        </a>
                    </div>
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-lg shadow-md p-6">
                <div class="flex items-center justify-between mb-6">
                    ...
                    Produits de {{ $craftsman->first_name }} {{ $craftsman->last_name }}
                </h3>
                    <span class="px-3 py-1 text-sm bg-blue-100 text-blue-800 rounded-full">
                            ...
                            <div class="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300">
                                ...
                                <div class="aspect-square bg-gradient-to-br from-gray-200 to-gray-300">
                                    @if($produit->main_image)
                                        <img src="{{ asset('storage/' . $produit->main_image) }}" alt="{{ $produit->name }}" class="w-full h-full object-cover">
                                    @else
                                        <img src="{{ asset('images/products/default-product.svg') }}" alt="{{ $produit->name }}" class="w-full h-full object-cover">
                                    @endif
                                </div>
                                
                                <!-- Informations du produit -->
                                <div class="p-4">
                                    <h4 class="font-semibold text-gray-900 mb-2">{{ $produit->name }}</h4>
                                    ...
                                    <div class="flex items-center justify-between mb-3">
                                        ...
                                        @endif
                                        <span class="text-lg font-bold text-blue-600">{{ number_format($produit->base_price, 2) }} €</span>
                                    </div>
                                    
                                    <!-- Matériau -->
                                    @if($produit->material)
                                        <p class="text-xs text-gray-500 mb-3">Matériau : {{ $produit->material }}</p>
                                    @endif
                                </div>
                            </div>
                        @endforeach
